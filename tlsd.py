...
def task3(): 
 ...
 end_time=time.time()
 print("\ntask3 spent:",end_time-start_time,"s")
def task4():
 start_time=time.time()
...
task2=executor.submit(task2)
task3=executor.submit(task3)
task4=executor.submit(task4)
task5=executor.submit(task5)
concurrent.futures.wait([task5,task4,task3,task2,task1])
...
full_precision_str = params['user_arguments'] + " --disable-safe-unpickle --xformers --enable-insecure-extension-access --opt-sub-quad-attention --opt-channelslast --api --port 60088 --cloudflared "
